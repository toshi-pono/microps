...
static void udp_pcb_release(struct udp_pcb *pcb) {
  ...
  pcb->local.port = 0;
  while (1) { /* Discard the entris in the queue. */
...
ssize_t udp_output(struct ip_endpoint *src, struct ip_endpoint *dst,
  ...
  if (ip_output(IP_PROTOCOL_UDP, (uint8_t *)buf, len + sizeof(*hdr), src->addr,
                dst->addr) == -1) {
...
int udp_bind(int id, struct ip_endpoint *local) {
  ...
  if (exist) {
    errorf("udp_pcb already exists (id=%d, local=%s, exist=%s)", id,
