...
static struct arp_cache *arp_cache_alloc(void) {
  ...
  for (entry = caches; entry < tailof(caches); entry++) {
    if (entry->state == ARP_CACHE_STATE_STATIC) continue;
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  if (ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER || msg->hdr.hln != ETHER_ADDR_LEN) {
    errorf("Hardware not match Ethernet: (hrd=0x%04x, hln=%u)",
           ntoh16(msg->hdr.hrd), msg->hdr.hln);
    return;
  }
  if (ntoh16(msg->hdr.pro) != ARP_PRO_IP || msg->hdr.pln != IP_ADDR_LEN) {
    errorf("Protocol not match IP: (pro=0x%04x, pln=%u)", ntoh16(msg->hdr.pro),
           msg->hdr.pln);
    return;
  }
...
int arp_init(void) {
  struct timeval interval = {1, 0}; /* ls */
